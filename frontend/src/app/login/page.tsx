...
export default function LoginPage() {
  ...
  const handleDevLogin = async () => {
    ...
    try {
      ...
      const response = await authAPI.login('dev', mockToken);
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.error || 'Dev mode login failed');
      }
    } catch (err: any) {
  ...
  const handleWalletLogin = async () => {
    ...
    try {
      const response = await authAPI.login('wallet', currentAccount.address);
      ...
      if (err.code === 'ERR_NETWORK' || err.message?.includes('ERR_CONNECTION_REFUSED')) {
        setError('Cannot connect to server, please ensure backend service is running (http://localhost:3001)');
      } else {
  ...
  // 監聽錢包連接狀態，自動登入
  useEffect(() => {
    if (currentAccount?.address && selectedRole && !loading && !connecting) {
      // 如果已經連接且有選擇角色，自動登入
      handleWalletLogin();
    }
  }, [currentAccount?.address, selectedRole]);
